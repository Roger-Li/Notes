...
get_z_star = function(alpha) {
    # qnorm is the inverse cdf, 
    # so what is the negative of the Z-score of the alpha/2 quantile of the normal distribution 
...
get_beta = function(z_star, s, d_min, N) {
    SE = s /  sqrt(N)
    # pnorm is the cdf,
    # so its the probability of (x/SE < z_star)
    # for a normal(d_min, SE) distribution
    # notice that z_star * SE is the margin of error
